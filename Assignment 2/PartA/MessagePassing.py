...
class Factor:
		...
		for i, j in enumerate(this_assignments):
			...
			for l, assn in enumerate(other_assignments):
				...
				if self.value[index] == 0.0 and other.value[l] == 0.0:
					...
					psi.value[index] = float(self.value[index]) * (1 / float(other.value[l]))
		return psi
	...
	def marginalise(self, V):
		...
		for i,j in enumerate(new_assignments):
			...
			for jump in jumps:
				...
				factor_sum += self.value[index]
			psi.value[i] = factor_sum
		return psi
	...
	def normalise(self):
		...
		# unnorm_max = max(self.value)
		# unnorm_min = min(self.value)
		# if unnorm_max / unnorm_min > 10.0e30:
		# 	self.value = [value * 10.0e30 for value in self.value]


		# elif unnorm_sum < 10.0e-10:
		# 	self.value = [value * 10.0e30 for value in self.value]
		# else:
...
class CRF:
	...
	def construct_markov_network(self, image_vectors, model):
		...
		for i in range(full_length):
			assoc_potens[i].append((copy.deepcopy(self.emission_probs[full_vector[i]]), i))
			if model != 'ocr':
				if (i != length1 - 1) and (i != full_length - 1):
					markov_network[i].add(i+1)
					markov_network[i+1].add(i)
					assoc_potens[i].append((copy.deepcopy(self.transmission_probs), i+1))
					...
					if i < length1:
						for j in range(i+1, length1):
							if full_vector[i] == full_vector[j]:
								...
								assoc_potens[i].append((copy.deepcopy(self.skip_factor), j))
								assoc_potens[j].append((copy.deepcopy(self.skip_factor), i))
						if model != 'ocr' and model != 'trans' and model != 'skip':					
							for j in range(length1, full_length):
								if full_vector[i] == full_vector[j]:
									markov_network[i].add(j)
									markov_network[j].add(i)
									assoc_potens[i].append((copy.deepcopy(self.pair_skip_factor), j))
									assoc_potens[j].append((copy.deepcopy(self.pair_skip_factor), i))
					else:
						for j in range(i+1, full_length):
							if full_vector[i] == full_vector[j]:
								markov_network[i].add(j)
								markov_network[j].add(i)
								assoc_potens[i].append((copy.deepcopy(self.skip_factor), i))
								assoc_potens[j].append((copy.deepcopy(self.skip_factor), i))
		...
		while var_elim_end is not True:
			...
			for i, node in enumerate(clique_tree):
				if i not in out_done and ((next_var in node.scope) or (var_elim_end is True and len(new_node.scope & node.scope) != 0)):
					...
					clique_tree[i].nbrs[j] = copy.deepcopy(sep_set)
					new_node.nbrs[i] = copy.deepcopy(sep_set)
					out_done.add(i)
		...
		for clique in clique_tree:
			for var in clique.scope:
				if var not in prediction:
					...
					unnorm_sum = sum(marginal.value)
					normalized_probs = [val/unnorm_sum for val in marginal.value]
	...
	def construct_factor_graph(self, image_vectors, model):
		...
		for i in range(full_length):
			new_node = CliqueTreeNode()
			factor_graph.append(new_node)
			factor_graph[i].scope.add(i)
			factor_graph[i].assoc_potens = copy.deepcopy(self.emission_probs[full_vector[i]])
			...
			factor_graph[i].belief.value = copy.deepcopy(self.emission_probs[full_vector[i]])
		for i in range(full_length):
			if model != 'ocr':
				if (i != length1 - 1) and (i != full_length - 1):
					...
					new_node = CliqueTreeNode()
					factor_graph.append(new_node)
					factor_graph[-1].assoc_potens = copy.deepcopy(self.transmission_probs)
					...
					factor_graph[-1].belief.value = copy.deepcopy(self.transmission_probs)
					...
					if i < length1:
						for j in range(i+1, length1):
							if full_vector[i] == full_vector[j]:
								...
								new_node = CliqueTreeNode()
								factor_graph.append(new_node)
								factor_graph[-1].assoc_potens = copy.deepcopy(self.skip_factor)
								factor_graph[-1].belief = Factor()
								...
								factor_graph[-1].belief.value = copy.deepcopy(self.skip_factor)
								factor_graph[-1].scope.add(i)
						...
						if model != 'ocr' and model != 'trans' and model != 'skip':					
							for j in range(length1, full_length):
								if full_vector[i] == full_vector[j]:
									...
									new_node = CliqueTreeNode()
									factor_graph.append(new_node)
									factor_graph[-1].assoc_potens = copy.deepcopy(self.pair_skip_factor)
									factor_graph[-1].belief = Factor()
									factor_graph[-1].belief.scope = set([i,j])
									factor_graph[-1].belief.stride = {i:1, j:self.n_chars}
									factor_graph[-1].belief.card = {i:self.n_chars, j:self.n_chars}
									factor_graph[-1].belief.value = copy.deepcopy(self.pair_skip_factor)
									factor_graph[-1].scope.add(i)
									factor_graph[-1].scope.add(j)
									factor_graph[-1].nbrs[i] = i
									factor_graph[-1].nbrs[j] = j
	...
	def belief_propagation(self, factor_graph, separators, epsilon = 1.0e-10, use_max = False):
		previous_separators = {}
		previous_beliefs = []
		initial_beliefs = []
